...
public class OrganizacaoInfoDao extends Dao<OrganizacaoInfo> {
	...
	private final String sqlOrganizacaoInfo = "SELECT ORGANIZACAOINFO.organizacao_id, ORGANIZACAOINFO.nome, ORGANIZACAOINFO.empresa_id"
			+ ",FROM ORGANIZACAOINFO (NOLOCK)";
	...
	public Collection<OrganizacaoInfo> buscaAllOrganizacaoInfo() {
		...
		try {
			...
			while (rsOrganizacaoInfo.next()) {
				...
				infoOrganizacao.setOrganizacao_id(rsOrganizacaoInfo
	...
	public Collection<OrganizacaoInfo> buscaOrganizacaoInfoByNome(String nome) {
		String sql = sqlOrganizacaoInfo;
		if (nome != null)
			sql += " WHERE ORGANIZACAOINFO.nome like ?";
		this.conn = this.conexao.getConexao();
		Collection<OrganizacaoInfo> infosOrganizacao = new ArrayList<OrganizacaoInfo>();
		try {
			this.stmt = conn.prepareStatement(sql);
	...
	public OrganizacaoInfo buscaOrganizacaoInfoByEmOrNo(Long empresa,
