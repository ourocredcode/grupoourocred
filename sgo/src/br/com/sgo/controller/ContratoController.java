...
public class ContratoController {
	...
	public ContratoController(Result result, BancoDao bancoDao,
			ProdutoBancoDao produtoBancoDao, ProdutoDao produtoDao,
			CoeficienteDao coeficienteDao, Contrato contrato,
			Formulario formulario, TabelaDao tabelaDao,
			ContratoDao contratoDao, FormularioDao formularioDao,
			WorkflowEtapaDao workFlowetapaDao, UsuarioInfo usuarioInfo,
			PeriodoDao periodoDao, TipoLogisticaDao tipoLogisticaDao,
			LogisticaDao logisticaDao, Empresa empresa,
			Organizacao organizacao, Usuario usuario,
			HistoricoContratoDao historicoContratoDao,
			HistoricoControleDao historicoControleDao, Controle boleto,
			Controle averbacao, ControleDao controleDao,
			TipoControleDao tipoControleDao) {
	...
	public void status(Long id) {
		...
		averbacao = this.controleDao.buscaControleByContratoTipoControle(id,
				tipoControleDao.buscaTipoControleByNome("Averbacao")
				.getTipoControle_id());

		contratos = contratoDao.buscaContratoByFormulario(formulario
				.getFormulario_id());

		if (boleto != null)
			historicoControleBoleto = this.historicoControleDao
			.buscaHistoricoByContratoControle(
					contrato.getContrato_id(), boleto.getControle_id());

		if (averbacao != null)
			historicoControleAverbacao = this.historicoControleDao
			.buscaHistoricoByContratoControle(
					contrato.getContrato_id(),
					averbacao.getControle_id());
	...
	public void altera(Contrato contrato) {
		...
		contrato.setCoeficiente(coeficienteDao.load(contrato.getCoeficiente()
				.getCoeficiente_id()));

		if (this.contrato.getBanco().getBanco_id() != contrato.getBanco()
				.getBanco_id()) {
			log.add("Banco alterado de : " + this.contrato.getBanco().getNome()
					+ " para: " + contrato.getBanco().getNome());
			this.contrato.setBanco(contrato.getBanco() == null ? null
					: contrato.getBanco());
		}

		if (this.contrato.getProduto().getProduto_id() != contrato.getProduto()
				.getProduto_id()) {
			log.add("Produto alterado: " + this.contrato.getProduto().getNome()
					+ " para: " + contrato.getProduto().getNome());
			this.contrato.setProduto(contrato.getProduto() == null ? null
					: contrato.getProduto());
		}

		if (this.contrato.getQtdParcelasAberto() != contrato
				.getQtdParcelasAberto()) {
			log.add("Parcelas Aberto alterado: "
					+ this.contrato.getQtdParcelasAberto() + " para : "
					+ contrato.getQtdParcelasAberto());
			this.contrato
			.setQtdParcelasAberto(contrato.getQtdParcelasAberto() == null ? null
					: contrato.getQtdParcelasAberto());
		}

		if (this.contrato.getPrazo() != contrato.getPrazo()) {
			log.add("Prazo alterado de : " + this.contrato.getPrazo()
					+ " para : " + contrato.getPrazo());
			this.contrato.setPrazo(contrato.getPrazo() == null ? null
					: contrato.getPrazo());
		}

		if (this.contrato.getCoeficiente().getCoeficiente_id() != contrato
				.getCoeficiente().getCoeficiente_id()) {
			log.add("Coeficiente alterado de : "
					+ this.contrato.getCoeficiente().getValor() + " para : "
					+ contrato.getCoeficiente().getValor());
			this.contrato
			.setCoeficiente(contrato.getCoeficiente() == null ? null
					: contrato.getCoeficiente());
		}

		if (this.contrato.getValorContrato().compareTo(
				contrato.getValorContrato()) != 0) {
			log.add("Valor contrato alterado de : "
					+ this.contrato.getValorContrato() + " para : "
					+ contrato.getValorContrato());
			this.contrato
			.setValorContrato(contrato.getValorContrato() == null ? null
					: contrato.getValorContrato());
		}

		if (this.contrato.getValorLiquido().compareTo(
				contrato.getValorLiquido()) != 0) {
			log.add("Valor Liquido alterado de : "
					+ this.contrato.getValorLiquido() + " para: "
					+ contrato.getValorLiquido());
			this.contrato
			.setValorLiquido(contrato.getValorLiquido() == null ? null
					: contrato.getValorLiquido());
		}

		if (this.contrato.getValorParcela().compareTo(
				contrato.getValorParcela()) != 0) {
			log.add("Valor Parcela alterado de : "
					+ this.contrato.getValorParcela() + " para: "
					+ contrato.getValorParcela());
			this.contrato
			.setValorParcela(contrato.getValorParcela() == null ? null
					: contrato.getValorParcela());
		}

		if (this.contrato.getValorMeta().compareTo(contrato.getValorMeta()) != 0) {
			log.add("Valor Meta Alterado de : " + this.contrato.getValorMeta()
					+ " para : " + contrato.getValorMeta());
			this.contrato.setValorMeta(contrato.getValorMeta() == null ? null
					: contrato.getValorMeta());
		}

		if (!(this.contrato.getValorDivida() == null && contrato
				.getValorDivida() == null)) {
			if (!(this.contrato.getValorDivida().compareTo(
					contrato.getValorDivida() == null ? null : contrato
							.getValorDivida()) == 0)) {
				log.add("Valor Divida alterado de : "
						+ this.contrato.getValorDivida() + " para : "
						+ contrato.getValorDivida());
				this.contrato
				.setValorDivida(contrato.getValorDivida() == null ? null
						: contrato.getValorDivida());
			}
		}

		if (!(this.contrato.getValorSeguro() == null && contrato
				.getValorSeguro() == null)) {
			if (!(this.contrato.getValorSeguro().compareTo(
					contrato.getValorSeguro() == null ? null : contrato
							.getValorSeguro()) == 0)) {
				log.add("Valor Seguro alterado de : "
						+ this.contrato.getValorSeguro() + " para : "
						+ contrato.getValorSeguro());
				this.contrato
				.setValorSeguro(contrato.getValorSeguro() == null ? null
						: contrato.getValorSeguro());
			}
		}

		if (!(this.contrato.getDesconto() == null && contrato.getDesconto() == null)) {
			if (!(this.contrato.getDesconto().compareTo(
					contrato.getDesconto() == null ? null : contrato
							.getDesconto()) == 0)) {
				log.add("Valor Desconto alterado de : "
						+ this.contrato.getDesconto() + " para : "
						+ contrato.getDesconto());
				this.contrato.setDesconto(contrato.getDesconto() == null ? null
						: contrato.getDesconto());
			}
		}
	...
	public void alteraStatus(Contrato contrato) {
		...
		if (!(this.contrato.getWorkflowEtapa().getWorkflowEtapa_id() == contrato
				.getWorkflowEtapa().getWorkflowEtapa_id())) {
			log.add("Status alterado de : "
					+ this.contrato.getWorkflowEtapa().getNome() + " para : "
					+ contrato.getWorkflowEtapa().getNome());
			this.contrato
			.setWorkflowEtapa(contrato.getWorkflowEtapa() == null ? null
					: contrato.getWorkflowEtapa());
		}
