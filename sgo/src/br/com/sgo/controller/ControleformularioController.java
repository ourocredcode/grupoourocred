		...
		for(Contrato c : formulario.getContratos()){
			...
			if(c.getEtapa().getNome().equals("Aguardando Pós Venda") || c.getEtapa().getNome().equals("Aguardando Qualidade")){
				...
				if(!status.equals("Recalcular")){
					status = ( c.getProduto().getNome().equals("CARTAO CREDITO") || c.getProduto().getNome().equals("SEGURO VIDA") ) && status.equals("Aguardando Logística") ? "Aguardando Digitação" : "Aguardando Logística";
				}
